\documentclass[a4paper,12pt]{book}


% \counterwithout{figure}{chapter} % pour compter la Figure 1 au lieu de Figure 1.1
%====================== PACKAGES ======================
\usepackage{enumitem}
\usepackage[round,authoryear]{natbib}
\makeatletter
\def\NAT@nmfmt#1{{\scshape\NAT@up#1}}
\makeatother
\usepackage[utf8]{inputenc}
\usepackage[greek.ancient,russian,french]{babel}
\usepackage{fontspec}
\babelfont[greek]{rm}[Scale=MatchLowercase,Script=Greek]{Gentium Plus}
\babelfont[russian]{rm}[Scale=MatchLowercase,Script=Cyrillic]{Gentium Plus}

\usepackage[T2A,LGR,T1]{fontenc}
\usepackage{lmodern}
%\usepackage{gentium}
\setmainfont{Gentium Plus}

\usepackage[x11names,table,xcdraw,dvipsnames]{xcolor}
\definecolor{maroon}{cmyk}{0,0.87,0.68,0.32}
\usepackage{soul}
\frenchsetup{StandardItemLabels=true}
%pour gérer les positionnement d'images
\usepackage{float}



\usepackage{amsmath}
\usepackage{graphicx}
\graphicspath{ {figures/} }
\usepackage{adjustbox}
\usepackage[colorinlistoftodos]{todonotes}
\setlength{\marginparwidth}{2cm} % Adjust marginpar width if needed
\usepackage{multirow}

\usepackage{listings}

% Set Monaco as the monospaced font
\setmonofont{Monaco}[Scale=0.85]
\usepackage{minted}
\floatname{listing}{Code}


\definecolor{codebg}{rgb}{0.98,0.98,0.98}

\usemintedstyle{colorful} % styles dispo : vs, tango, friendly, colorful, etc.
\lstdefinelanguage{XML}
{
	morestring=[b]",
	morecomment=[s]{<!--}{-->},
	stringstyle=\color{Bittersweet},
	identifierstyle=\color{Blue},
	keywordstyle=\color{brown},
	morekeywords={version,encoding,type,href,xmlns,when}
}

\lstset{
	language=XML,
	basicstyle=\footnotesize\ttfamily,
	breaklines=true,
	columns=fullflexible,
	keepspaces=true,
	showstringspaces=false,
	frame=single,
	backgroundcolor=\color{white},
	numbers=left,
	numberstyle=\tiny\color{gray},
	stepnumber=1,
	numbersep=10pt,
	xleftmargin=15pt,
	tabsize=2
}

	


\newcommand{\smalltodo}[2][]{\todo[size=\scriptsize, color=white, linecolor=blue, textcolor=black #1]{#2}}

\usepackage{url}
%pour les informations sur un document compilé en PDF et les liens externes / internes
%pour la mise en page des tableaux
\usepackage{array}
\usepackage{tabularx}
%\usepackage[round,authoryear]{natbib}
%pour utiliser \floatbarrier
%\usepackage{placeins}
%\usepackage{floatrow}
%espacement entre les lignes
\usepackage{setspace}
\onehalfspacing
%modifier la mise en page de l'abstract
%\usepackage{abstract}
%\renewcommand{\abstractnamefont}{\normalfont\Large\bfseries}
%police et mise en page (marges) du document
%\usepackage{polyglossia}
%\setmainlanguage{french}
%\setotherlanguage[variant=polytonic]{greek}
%\newfontfamily{\greekfont}[Ligatures=TeX]{Linux Libertine O}
%\usepackage{fontspec}
%\setmainfont{DejaVu Serif}
\usepackage[top=2cm, bottom=2cm, left=3cm, right=3cm]{geometry}
%Pour les galerie d'images
%\usepackage{subfig}
\usepackage{fancyhdr}
\usepackage{pdflscape}
\pagestyle{fancy}
%\fancyhf{}
\fancyhead[L]{\rightmark}
\fancyhead[R]{\thepage}

\renewcommand{\headrulewidth}{0.4pt}% Default \headrulewidth is 0.4pt
\renewcommand{\footrulewidth}{0.4pt}% Default \footrulewidth is 0pt
\usepackage[labelfont=bf]{caption}

\captionsetup{justification=centering} % Justifie toutes les légendes
\captionsetup[sub]{justification=justified}
% Define the custom page style
\fancypagestyle{plain}{%
	\fancyhf{}%
	\renewcommand{\headrulewidth}{0pt}%
	\renewcommand{\footrulewidth}{0pt}%
}

\usepackage{subcaption}


\captionsetup{justification=centering,font=small}
% renommer « Fig. » en « Figure »
\addto\captionsfrench{\renewcommand{\figurename}{\textsc{Figure}}}

\addto\captionsfrench{\def\tablename{Tableau}}

\usepackage{lipsum} 
\usepackage{blindtext, color}
\usepackage[explicit]{titlesec}





\usepackage{titlesec}

\usepackage{fmtcount} % pour écrire les nombres en lettres

% Fonction pour écrire "Première", "Deuxième", etc.
\newcommand{\partnamefr}{%
	\ifcase\value{part}%
	\or Première%
	\or Deuxième%
	\or Troisième%
	\or Quatrième%
	\or Cinquième%
	\or Sixième%
	\or Septième%
	\or Huitième%
	\or Neuvième%
	\or Dixième%
	\else Partie~\number\value{part}%
	\fi~partie%
}

\titleformat{\part}
[display]
{\normalfont\Huge\bfseries\centering}
{\MakeUppercase{\partnamefr}}
{1ex}
{%
	\rule{\textwidth}{0.4pt}\\[1ex]%
	\Huge\textsc{#1}%
}
%======================================%
% Rétablir la numérotation hiérarchique
%======================================%
%\makeatletter
%\renewcommand{\thesection}{\arabic{section}}
%\renewcommand{\thesubsection}{\thesection.\arabic{subsection}} % optional
%\renewcommand{\@seccntformat}[1]{\csname the#1\endcsname\quad}
%\makeatother

\definecolor{gray75}{gray}{0.35}
%\newcommand{\hsp}{\hspace{20pt}}
%\titleformat{\chapter}[hang]{\Huge\bfseries}{Chapitre \thechapter\hsp\textcolor{gray75}{|}\hsp}{0pt}{\Huge\bfseries}

% How can I have a vertical bar in chapter title which scales with the broken title?
\newcommand{\hsp}{\hspace{16pt}}
\titleformat{\chapter}[hang]{\LARGE}{\color{gray75}\textsc{Chapitre} \thechapter}{14pt}{\begin{tabular}[t]{@{\color{gray75}\hsp}p{0.75\textwidth}}\raggedright\textsc{#1}\end{tabular}\\\hrule}


\usepackage{gensymb} % pour le symbole du degré (\degree)

\usepackage[
left = \flqq{},% 
right = \frqq{},% 
leftsub = \flq{},% 
rightsub = \frq{} %
]{dirtytalk}

%====================== INFORMATION ET REGLES ======================
\usepackage{minitoc}
\setcounter{minitocdepth}{2}







%change TOC header
\addto{\captionsfrench}{% Making babel aware of special titles
	\renewcommand{\mtctitle}{Sommaire}
}



%rajouter les numérotation pour les \paragraphe et \subparagraphe
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}

% ajouter de l'espace entre le numéro de la footnote et le texte de la footnote
\usepackage[hang]{footmisc}
\setlength{\footnotemargin}{2mm}
%\usepackage[linkbordercolor={0 0 1}]{hyperref}
% pour les références croisées (page X pour une référence dans la bibliographie)
\usepackage[backref=page]{hyperref} 

\renewcommand\backreftwosep{, }
\renewcommand\backrefsep{, }
\renewcommand*{\backrefalt}[4]{%
	\ifcase #1%
	\or (page~#2)%
	\else (pages~#2)%
	\fi%
}


\usepackage{etoolbox}
\makeatletter
\patchcmd{\cleardoublepage}{\thispagestyle{empty}}{}{}{}
\makeatother




% Patch \NAT@citex to suppress initials
\makeatletter
\patchcmd{\NAT@citex}
{\ifNAT@swa\else\if*#2*\else\NAT@cmt#2\fi
	\if\relax\NAT@date\relax\else\NAT@@close\fi\fi}
{\ifNAT@swa\else\if*#2*\else\NAT@cmt#2\fi\fi}
{}{}
\patchcmd{\NAT@citex}
{\def\NAT@numclose{\@citeb}\def\@citea{\NAT@separator}}
{\def\NAT@numclose{\@citeb}\def\@citea{\NAT@separator}}
{}{}
\makeatother

% pour les backrefs : ça ne marche pas encore, à voir
%\usepackage[backref=page]{hyperref}
%\makeatletter
%\patchcmd{\BR@backref}{\newblock}{\newblock(page~}{}{}
%\patchcmd{\BR@backref}{\par}{)\par}{}{}
%\makeatother 


% pour que les références soient complètement encadrées
\makeatletter

\pretocmd{\NAT@citex}{%
	\let\NAT@hyper@\NAT@hyper@citex
	\def\NAT@postnote{#2}%
	\setcounter{NAT@total@cites}{0}%
	\setcounter{NAT@count@cites}{0}%
	\forcsvlist{\stepcounter{NAT@total@cites}\@gobble}{#3}}{}{}
\newcounter{NAT@total@cites}
\newcounter{NAT@count@cites}
\def\NAT@postnote{}

% include postnote and \citet closing bracket in hyperlink
\def\NAT@hyper@citex#1{%
	\stepcounter{NAT@count@cites}%
	\hyper@natlinkstart{\@citeb\@extra@b@citeb}#1%
	\ifnumequal{\value{NAT@count@cites}}{\value{NAT@total@cites}}
	{\ifNAT@swa\else\if*\NAT@postnote*\else%
		\NAT@cmt\NAT@postnote\global\def\NAT@postnote{}\fi\fi}{}%
	\ifNAT@swa\else\if\relax\NAT@date\relax
	\else\NAT@@close\global\let\NAT@nm\@empty\fi\fi% avoid compact citations
	\hyper@natlinkend}
\renewcommand\hyper@natlinkbreak[2]{#1}

% avoid extraneous postnotes, closing brackets
\patchcmd{\NAT@citex}
{\ifNAT@swa\else\if*#2*\else\NAT@cmt#2\fi
	\if\relax\NAT@date\relax\else\NAT@@close\fi\fi}{}{}{}
\patchcmd{\NAT@citex}
{\if\relax\NAT@date\relax\NAT@def@citea\else\NAT@def@citea@close\fi}
{\if\relax\NAT@date\relax\NAT@def@citea\else\NAT@def@citea@space\fi}{}{}

\makeatother

\gappto{\UrlBreaks}{\UrlOrds}
\hypersetup{							% Information sur le document
	pdfauthor = {Premier Auteur,
		Deuxième Auteur,
		Troisième Auteur,
		Quatrième Auteur},			% Auteurs
	pdftitle = {Nom du Projet -
		Sujet du Projet},			% Titre du document
	pdfsubject = {Mémoire de Projet},		% Sujet
	pdfkeywords = {Tag1, Tag2, Tag3, ...},	% Mots-clefs
	pdfstartview={FitH},
	colorlinks=true,
	linkcolor=black,
	filecolor=magenta,      
	urlcolor=blue,
	citecolor=black,
	linkbordercolor={0 0 1},
	citebordercolor=red,
	urlbordercolor=red,
	pdftitle={Overleaf Example},
	pdfpagemode=FullScreen}					% ajuste la page à la largueur de l'écran

\usepackage{multicol}

\AtBeginDocument{\hypersetup{pdfborder={0 0 1}}}
\setpdflinkmargin{2pt}
%pdfcreator = {MikTeX},% Logiciel qui a crée le document
%pdfproducer = {}} % Société avec produit le logiciel
\usepackage{fancyhdr}% http://ctan.org/pkg/fancyhdr
% \pagestyle{fancy}% Change page style to fancy
% \fancyhf{}% Clear header/footer
% \fancyhead[C]{}
% \fancyfoot[C]{}% \fancyfoot[R]{\thepage}

\usepackage{colortbl}

\usepackage{longtable}
\newenvironment{myepigraph}
{\par\hfill\itshape
\begin{tabular}{@{}r@{\hspace{2em}}}} % 2em from the right margin
{\end{tabular}\par\medskip}


\DeclareTextSymbol{\CYRDJE}{T2A}{130}

% prevent LaTeX from adding excessive white space:
\raggedbottom
\usepackage{booktabs}
\usepackage{tikz}
\usetikzlibrary{shapes.geometric, arrows.meta, positioning}

\tikzstyle{process} = [rectangle, rounded corners, minimum width=3.5cm, minimum height=1cm, text centered, draw=black, fill=blue!10]
\tikzstyle{dataset} = [rectangle, dashed, minimum width=2.8cm, minimum height=1cm, text centered, draw=black, fill=gray!15]
\tikzstyle{arrow} = [thick, -{Stealth}]

\usepackage{array}
\usepackage{ragged2e}

\usepackage{setspace} % pour \setstretch

\usepackage[most]{tcolorbox}



%======================== DEBUT DU DOCUMENT ========================


\begin{document}
\dominitoc
\reversemarginpar % For \marginpar on the left

%régler l'espacement entre les lignes
\newcommand{\HRule}{\rule{\linewidth}{0.5mm}}
\onehalfspacing
%page de garde
%\input{title.tex} % ici mettre la page de garde de SU

%page blanche
%%%%%%%%%%%% décommenter la partie suivante jusqu'à \tableofcontents pour afficher les remerciements et le résumé
%\newpage
%~
%%ne pas numéroter cette page
%\thispagestyle{empty}
%\newpage
%\setcounter{page}{0}
%\input{remerciements}
%\thispagestyle{empty}
%\setcounter{page}{0}
%%ne pas numéroter le sommaire
%\newpage
%~
%\thispagestyle{empty}
%\setcounter{page}{0}
%%ne pas numéroter le sommaire
%
%\newpage
%
%\input{0_abstract}
%\thispagestyle{empty}
%\setcounter{page}{0}
%%ne pas numéroter le sommaire
%\newpage
%~
%\thispagestyle{empty}
%\setcounter{page}{0}
%%ne pas numéroter le sommaire
%
%\newpage
%%%%%%%%%%%%%% décommenter la partie au-dessus, jusqu'à \newpage pour afficher les remerciements et le résumé 



% ================== Résumé ==================
\cleardoublepage
\pagestyle{empty}
% Commande pour un chapitre centré sans numéro
% Commande propre pour chapitre centré sans numéro




% Remove page numbers for the Table of Contents and List of Figures

\clearpage
\begingroup
%\let\origplain\ps@plain
\def\ps@plain{\ps@empty} % Redefine plain as empty temporarily
\pagestyle{empty}
\pdfbookmark{\contentsname}{toc}
\tableofcontents

% Décommenter cette partie pour révéler les listes de figures, de tableaux et des codes
%\clearpage
%\listoffigures
%\clearpage
%\listoftables
%\clearpage
%\listof{listing}{Liste des codes}
%%\let\ps@plain\origplain % Restore original plain page style
%\endgroup




% Revenir au style habituel
\clearpage
\pagestyle{fancy}
\setcounter{page}{1}







%espacement entre les lignes d'un tableau
\renewcommand{\arraystretch}{1.5}

%====================== INCLUSION DES PARTIES ======================

~
\thispagestyle{empty}
%recommencer la numérotation des pages à "1"
\setcounter{page}{0}
\newpage

\input{1_intro}
\input{2_rupture}
\input{3_methodo}
\input{2_SotA}
\input{methodo_sota_globale}
\input{structure}




%\part{La circulation des savoirs, moteur de la valorisation des archives : état de l'art}
\part{Mobiliser les technologies numériques pour étudier la circulation des savoirs : un champ de recherche en plein essor}

\input{revue_lit}




\part{Pister la circulation des savoirs médicaux au prisme du numérique -- le cas Charcot}
\input{4_resultats}



%%\addcontentsline{toc}{chapter}{Conclusion}
\input{5_conclusion}
%%



\appendix
%\let\cleardoublepage\clearpage % avoid blank page before

\clearpage
\thispagestyle{empty}
\begin{center}
	\vspace*{\fill} % push down to vertical center
	{\LARGE\bfseries Annexe}
	\vspace*{\fill} % push up from bottom
\end{center}

\addcontentsline{toc}{chapter}{Annexe}

\clearpage % optional: start annex content on next page
\input{6_annexe}

\cleardoublepage
\addcontentsline{toc}{chapter}{Bibliographie}
\bibliographystyle{apalike-etal-in-italics}
\bibliography{bibliographie.bib}




\newcommand{\centredchapter}[1]{%
	\cleardoublepage
	\thispagestyle{empty}
	\vspace*{2cm} % Ajuste l'espace pour descendre le titre
	\begin{center}
		\normalfont\Large\bfseries #1
	\end{center}
	\vspace{1cm} % Ajuste l'espace entre le titre et le texte
	\addcontentsline{toc}{chapter}{#1}%
}



\cleardoublepage
\pagestyle{empty}

\centredchapter{Résumé}

\input{0_abstract}



%voir wiki pour plus d'information sur la syntaxe des entrées d'une bibliographie


\end{document}